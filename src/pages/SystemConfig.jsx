...
const TABS = [
  ...
  ['users', 'User Accounts', 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z'],
...
// User Accounts panel (62) — administrators reset a user's password to the
// deployment default. Cloud-mode only: accounts live in D1, so local demo
// mode shows an explanatory notice instead of the list.
function UsersPanel({ flashSaved }) {
  ...
  const load = () => {
    setLoadError('')
    Promise.all([api('/api/admin/users'), api('/api/admin/default-password')])
      .then(([u, d]) => {
        setUsers(u.users || [])
        setDefaultPassword(d.password || '')
      })
      .catch((e) => setLoadError(e.message || 'Failed to load user accounts.'))
  }
  ...
  const confirmReset = async () => {
    ...
    setModalError('')
    try {
      await api('/api/admin/users/reset-password', { method: 'POST', body: { email: target.email } })
      setUsers((prev) => prev.map((u) => (u.email.toLowerCase() === target.email.toLowerCase() ? { ...u, must_change_password: 1, usingDefaultPassword: true } : u)))
      setTarget(null)
      flashSaved?.()
    } catch (e) {
      setModalError(e.message || 'Failed to reset the password.')
    } finally {
      setBusy(false)
    }
  }
  ...
  if (!apiEnabled()) {
    return (
      <div className="space-y-5">
        {header('User Accounts', 'Reset a user\'s password to the deployment default.')}
        <p className="rounded-lg bg-amber-50 px-4 py-3 text-xs font-medium leading-relaxed text-amber-800 ring-1 ring-amber-200">
          User accounts are managed in the cloud database. Connect the app to the server API to reset passwords here.
        </p>
      </div>
    )
  }
  ...
  const visible = (users || []).filter((u) => !q || String(u.name || '').toLowerCase().includes(q) || String(u.email || '').toLowerCase().includes(q))

  return (
    <div className="space-y-5">
      {header('User Accounts', 'Reset a user\'s password to the deployment default. They will be asked to choose a new one at next sign-in.')}

      {loadError && (
        <div className="flex items-start gap-2 rounded-lg bg-red-50 px-4 py-3 text-xs font-medium text-red-700 ring-1 ring-red-200" role="alert">
          ...
          {loadError}
          <button type="button" onClick={load} className="ml-auto shrink-0 rounded-lg border border-red-200 px-3 py-1.5 font-semibold text-red-700 hover:bg-red-100">Retry</button>
        </div>
      )}
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-9 pr-3 text-sm placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 sm:max-w-xs"
        />
      </div>

      {users === null && !loadError && (
        <p className="py-8 text-center text-sm text-gray-400">Loading user accounts…</p>
      )}

      {users !== null && visible.length === 0 && (
        <p className="rounded-lg bg-gray-50 px-4 py-6 text-center text-xs text-gray-500 ring-1 ring-gray-100">
          {q ? 'No accounts match your search.' : 'No user accounts yet — accounts are created when companies register.'}
        </p>
      )}
      ...
      {users !== null && visible.length > 0 && (
        <div className="divide-y divide-gray-100 rounded-xl border border-gray-200">
          {visible.map((u) => (
            <div key={u.email} className="flex flex-wrap items-center gap-3 p-4">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-600 text-xs font-bold text-white">
                {String(u.name || u.email).split(' ').map((n) => n[0]).slice(0, 2).join('').toUpperCase()}
              </div>
              <div className="min-w-0 flex-1">
                <p className="flex flex-wrap items-center gap-2 text-sm font-semibold text-gray-900">
                  <span className="truncate">{u.name || u.email}</span>
                  {u.role === 'administrator' && <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-gray-600">Platform</span>}
                  {u.usingDefaultPassword && u.role !== 'administrator' && (
                    <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-amber-700 ring-1 ring-amber-200">Default password</span>
                  )}
                </p>
                <p className="truncate text-xs text-gray-500">{u.email} · <span className="capitalize">{u.role || 'employee'}</span></p>
              </div>
              <button
                type="button"
                onClick={() => openReset(u)}
                disabled={u.role === 'administrator'}
                title={u.role === 'administrator' ? 'Platform accounts keep their dedicated credentials' : 'Reset to the default password'}
                className="min-h-[44px] shrink-0 rounded-lg border border-gray-300 bg-white px-4 py-2 text-xs font-semibold text-gray-700 transition hover:border-brand-400 hover:text-brand-700 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:border-gray-300 disabled:hover:text-gray-700"
              >
                Reset password
              </button>
            </div>
      ...
      {target && (
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4" onClick={() => !busy && setTarget(null)}>
          ...
          <div className="relative max-h-[92vh] w-full overflow-y-auto rounded-t-2xl bg-white p-6 shadow-xl sm:max-w-md sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-2 text-xs leading-relaxed text-gray-500">
              Set the password for <span className="font-semibold text-gray-800">{target.name || target.email}</span> (<span>{target.email}</span>) back to the
              deployment default. They will be required to choose a new password at their next sign-in.
            </p>
            <label className="mt-4 block text-xs font-medium text-gray-700">
              ...
              <span className="relative mt-1 block">
                <input
                  ...
                  readOnly
                  aria-label="Default password that will be applied"
                  className="w-full rounded-xl border border-gray-300 bg-gray-50 py-2.5 pl-3.5 pr-11 font-mono text-sm text-gray-900 focus:border-brand-500 focus:outline-none focus:ring-4 focus:ring-brand-500/10"
                />
                <button
                  type="button"
            ...
            <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2.5 text-xs font-medium leading-relaxed text-amber-800 ring-1 ring-amber-200">
              Share this password securely (in person or via a private channel) — the user should change it right away.
            </p>
            {modalError && (
              <p className="mt-3 rounded-lg bg-red-50 px-3 py-2.5 text-xs font-medium text-red-700 ring-1 ring-red-200" role="alert">{modalError}</p>
            )}
            <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setTarget(null)} disabled={busy} className="min-h-[44px] rounded-xl border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-50">Cancel</button>
              <button type="button" onClick={confirmReset} disabled={busy} className="min-h-[44px] rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white shadow hover:bg-brand-700 disabled:opacity-50">
                ...
                <div className="mt-4 rounded-xl border border-gray-200 bg-gray-50 p-4">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      ['tasks', 'Tasks', [['add', 'Add'], ['edit', 'Edit / Due date'], ['transfer', 'Transfer'], ['delete', 'Delete']]],
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-[260px_1fr] xl:grid-cols-[280px_1fr]">
        ...
        <div className="min-w-0 space-y-5 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6 lg:p-8">
          ...
          {tab === 'users' && <UsersPanel flashSaved={flashSaved} />}
