  ...
  return (
    <div className="mx-auto w-56 rounded-[2rem] border border-gray-200 bg-gray-900 p-2 shadow-xl">
      ...
      <div className="flex h-96 flex-col items-center justify-between rounded-[1.6rem] bg-gradient-to-b from-brand-600 to-emerald-500 px-4 py-6 text-white">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-brand-100">Unified Workforce</p>
        <div className="flex flex-col items-center gap-3">
          {method === 'fingerprint' && (
            <>
              ...
              <p className="text-xs font-medium text-brand-100">Touch the sensor to sign in</p>
            </>
          )}
          {method === 'pin' && (
            <>
          ...
          {method === 'qr' && (
            <>
              ...
              <p className="text-xs font-medium text-brand-100">Scan your employee QR badge</p>
            </>
          )}
        </div>
        <div className="w-full rounded-lg bg-white/10 py-2 text-center text-[10px] font-medium text-brand-100 ring-1 ring-white/20">
          Check In / Check Out
        </div>
      </div>
    </div>
    ...
    <form onSubmit={save} className="space-y-6">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Configure how employees authenticate at time-keeping kiosks.</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_auto]">
              ...
              {methods.map((m) => (
                <label
                  ...
                  className={`relative cursor-pointer rounded-xl border-2 p-4 transition ${
                    method === m.id ? 'border-brand-500 bg-brand-50/60' : 'border-gray-200 hover:border-gray-300'
                  }`}
                >
                  ...
                  <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${method === m.id ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
            ...
            {method === 'pin' && (
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <label className="block text-sm">
                  ...
                  <select defaultValue="4" className={inputCls}>
                ...
                <label className="block text-sm">
                  ...
                  <select defaultValue="5" className={inputCls}>
                    <option>3 attempts</option><option>5 attempts</option><option>No lockout</option>
                  </select>
                </label>
              </div>
            )}
            {method === 'qr' && (
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <label className="block text-sm">
                  ...
                  <select defaultValue="rear" className={inputCls}>
          ...
          <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <label className="block text-sm">
                ...
                <select defaultValue="60" className={inputCls}>
              ...
              <label className="block text-sm">
                ...
                <select defaultValue="hq" className={inputCls}>
                  <option value="hq">Head Office</option>
                  <option value="branch1">Branch 1 — Makati</option>
                  <option value="branch2">Branch 2 — Cebu</option>
                </select>
              </label>
            </div>
          </section>
        ...
        <aside className="self-start lg:sticky lg:top-24">
          <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-gray-400 text-center">Live Preview</p>
